...
def load_pretrained_model(config):
    ...
    if config['Model_Type'] == 'Swin-T':
        ...
        if not sto:
            state_dict = load_pretrained_swinT('model/rsp-swin-t-ckpt.pth',transferred_model)
        ...
        transferred_model = build_model('train',config['Model_Type'])
        if not sto:
            state_dict=load_pretrained_swinT('model/Swin-B/epoch91.pth', transferred_model)
            transferred_model.load_state_dict(state_dict,strict=False)
        num_feature = transferred_model.head.in_features
        del transferred_model.head
        transferred_model.head = nn.Linear(num_feature, config['cate_num'])
        ...
        if not sto:
            checkpoint = torch.load('model/Mobilev3/epoch95.pth', map_location='cpu') 
        ...
        if not sto:
            ...
            checkpoint = torch.load('model/rsp-resnet-50-ckpt.pth', map_location='cpu')  # 加载模型文件，pt, pth 文件都可以；
        ...
        if not sto:
            new_state_dict = OrderedDict()
            checkpoint = torch.load('model/resnet18_I_nwpu_cate45.pth', map_location='cpu')  # 加载模型文件，pt, pth 文件都可以；
        ...
        if not sto:
            new_state_dict = OrderedDict()
            checkpoint = torch.load('model/resnet101/NWPU-ResNet101.pth', map_location='cpu')  # 加载模型文件，pt, pth 文件都可以；
            new_state_dict = checkpoint
            transferred_model.load_state_dict(new_state_dict)
        transferred_model.fc = nn.Linear(num_feature, config['cate_num'])
        ...
        if not sto:
            new_state_dict = OrderedDict()
            checkpoint = torch.load('model/densenet121/NWPU-DenseNet121.pth', map_location='cpu')  
            new_state_dict = checkpoint
            transferred_model.load_state_dict(new_state_dict)
        ...
        if not sto:
            new_state_dict = OrderedDict()
            checkpoint = torch.load('model/Senet50/epoch297.pth', map_location='cpu')  # 加载模型文
            new_state_dict = checkpoint
            transferred_model.load_state_dict(new_state_dict)
        transferred_model.fc = nn.Linear(num_feature, config['cate_num'])
    else:
