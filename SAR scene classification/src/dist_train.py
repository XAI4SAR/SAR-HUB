...
from model_save_load import model_transfer, save_model, save_model_DDP
import torch.nn as nn
from torch.optim import lr_scheduler
from loss import  FocalLoss, CBCE_Loss, CB_Focal_Loss,  MINI_CBCE_Loss, MINI_CB_Focal_Loss
...
def get_dataloader(config):
    ...
    if config['dataset'] == 'TerraSAR-X':
        ...
        data_transforms = {
        'train': transforms.Compose([
            data_transform.truncated_train_ben(),
            data_transform.Normalize(nor_mean,nor_std),
            data_transform.Numpy2Tensor(3),
            transforms.Resize(128),
            transforms.RandomHorizontalFlip(0.5),
        ...
        data_transforms = {
        'train': transforms.Compose([
            data_transform.truncated_train_osu(),
            data_transform.Normalize(nor_mean,nor_std),
            data_transform.Numpy2Tensor(3),
            transforms.Resize(128),
            transforms.RandomHorizontalFlip(0.5),
...
def train_loss(config):
    ...
    elif config['loss_type'] == 'CB_Focal':
            criterion = CB_Focal_Loss(samples_per_cls=cls_num_list,beta = 0.995, num_of_cls=7, gamma = 2)
    elif config['loss_type'] == 'Mini_CB_Focal':
            criterion = MINI_CB_Focal_Loss(samples_per_cls=cls_num_list,beta = 0.995, num_of_cls=7, gamma = 2)
    else:
...
def resnet_train(config,world_size):
    ...
    for epoch in range(config['num_epochs']):
        ...
        if dist.get_rank() == 0 and config['if_save']==1:
