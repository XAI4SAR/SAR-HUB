...
def config_train():
    ...
    config['Model_Type'] = ' '     # Model Type: ResNet-18,ResNet-50,ResNet-101,
    config['loss_type'] = 'CE'   #! CE:cross entropy；FL：Focal Loss；CB_CE：Class-Balanced-CE；CB_FL：Class-Balanced-FL；Mini_CB_CE：Mini-Class-Balanced-CE；Mini_CB_FL：Mini-Class-Balanced-FL
    config['dataset'] = 'TerraSAR-X' #! TerraSAR-X、BigEarthNet-Small or OpenSARUrban
    
    
    if config['dataset'] == 'TerraSAR-X':
        config['datatxt_train'] = ' '
        config['datatxt_val'] = ' '
