...
def load_pretrained_swin(pretrain_model_path, model):
    ## isofficial: 判断模型是否为已有工作模型，目前已有工作的Vit模型大多以checkpoint['model']的方式存储模型权重，需要单独考虑
...
def load_pretrained_tiny(pretrain_model_path, model,isofficial):
    ## isofficial: 判断模型是否为已有工作模型，目前已有工作的Vit模型大多以checkpoint['model']的方式存储模型权重，需要单独考虑
    checkpoint = torch.load(pretrain_model_path, map_location='cpu')
    if isofficial:
        state_dict = checkpoint['model']
