...
class U_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # de_feature_2 = self.deconv_2(de_feature_1_conv) ## 512,16,16 -> 256,32,32
        # temp = torch.cat((feature_3, de_feature_2), dim=1) ## 256,32,32 -> 512,32,32
        # de_feature_2_conv = self.right_conv_2(temp)  ## 512,32,32 -> 256,32,32

        # de_feature_3 = self.deconv_3(de_feature_2_conv) ## 128,64,64

        # temp = torch.cat((feature_2, de_feature_3), dim=1) ## 256,64,64
        # de_feature_3_conv = self.right_conv_3(temp) ## 128,64,64

        # de_feature_4 = self.deconv_4(de_feature_3_conv) ## 64,128,128
        # temp = torch.cat((feature_1, de_feature_4), dim=1) ## 128,128,128
        # de_feature_4_conv = self.right_conv_4(temp) ## 64,128,128
        # out = self.right_conv_5(de_feature_4_conv)
        # out = self.right_conv_3(feature_3_pool)
